...
(function() {
    ...
    function isCarouselView() {
        try {
            // Wenn wir keine vollständige URL haben, ist es mit hoher Wahrscheinlichkeit ein Karussell
            if (!isFullStoryUrl()) {
                return true;
            }
    ...
    function redirectToSingleStoryView(username, storyId) {
        ...
        if (isAlreadyOnSingleViewUrl(username, storyId)) {
            log('Bereits auf der korrekten Einzelansicht-URL, keine Umleitung notwendig');
            isInSingleView = true;
            return;
        }
    ...
    function processSingleStoryView(retryCount = 0) {
        ...
        if (processAttempts >= MAX_PROCESS_ATTEMPTS) {
            ...
            // Zeige den Button nur, wenn wir in einer Karussell-Ansicht sind und keine Einzel-URL haben
            const isCarousel = isCarouselView();
            if (isCarousel && !isFullStoryUrl()) {
        ...
        try {
            ...
            // Prüfe, ob wir uns in einer Karussell-Ansicht befinden
            const carousel = isCarouselView();
            
            // Wenn wir bereits eine vollständige URL mit story_id haben und keine Karussell-Ansicht sehen,
            // sind wir bereits in der Einzelansicht
            if (urlStoryId && !carousel) {
            ...
            // Wenn wir ein Karussell erkennen oder keine Story-ID in der URL haben
            if (carousel || !urlStoryId) {
                if (carousel) {
                ...
                updateButtonState(carousel, domStoryId);
                
                if (domStoryId && !buttonClicked) {
                    log(`Story-ID aus DOM extrahiert: ${domStoryId}`);
                    isProcessing = false;
                    redirectToSingleStoryView(username, domStoryId);
                } else if (retryCount < MAX_RETRIES && !buttonClicked) {
    ...
    const observerCallback = (mutations) => {
        ...
        // Überprüfe, ob wir bereits in einer Einzelansicht sind und keine Änderungen mehr benötigen
        if (isInSingleView || buttonClicked) {
            return;
        }
    ...
    document.addEventListener('click', (event) => {
        ...
        setTimeout(() => {
            if (isStoryPage() && !isProcessing && !buttonClicked && !isInSingleView) {
